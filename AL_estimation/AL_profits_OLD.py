...
def ghat_i(v, h, bid):  # this is to get the variance of the estimator
    def gauss_kernel(x):
        return (1/(np.sqrt(2*np.pi)*h)) * np.exp(-0.5*((x/h)**2))
    return gauss_kernel((v - bid) / h)
...
def compute_expected_profit_KDE(n, r, max_bid_dicts, ub_v, v0, variedN=False, KDEs=None):
    ...
    ub = integrate.quad(f_ub,0,75)[0] - v0 - fnn_l * (r - v0)
    lb = integrate.quad(f_lb,0,75)[0] - v0 - fnn_u * (r - v0)

    # inference
    ci_lb, ci_ub = ALinf.CI_out(ub, lb, L, alpha=0.05)

    return lb,ub,ci_lb,ci_ub

def compute_expected_profit_KDE_barN(n, r, max_bid_dicts, ub_v, v0, barN, variedN=False, KDEs=None):
    v_n1n, ghat_KDE_n1n,h = ghat_KDE(max_bid_dicts,n-1,n, ker='gaussian', bandwidth="ISJ")
    v_nn, ghat_KDE_nn,h = ghat_KDE(max_bid_dicts,n,n, ker='gaussian', bandwidth="ISJ")
...
# Method (not as accurate): Using stieltjes integral
