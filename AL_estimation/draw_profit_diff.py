...
def draw_profit_diff_specificN(INPATH, OUTPATH, n, ub_v=10, num_points=1000):
    ...
    plt.ylabel("Expected Profit Difference")
    # plt.ylim(-0.1, 1)
    plt.plot(X,diff,color='black',linewidth=2, label='non-equilibrium')
    plt.plot(X,diff_ub,color='blue',linewidth=2, label='ub')
    plt.plot(X,diff_lb,color='blue',linewidth=2, label='lb')
    plt.legend()
...
print(OUTPATH)
draw_profit_diff_specificN(INPATH, OUTPATH, n=3, UB_V=5, num_points=300)
