...
def gen_many_simulations(n, lbda, increment, distribution='lognormal', num_simulations=10000):
    out_list = []
    for i in tqdm(range(num_simulations)):
        out_list.append(run_1_simulation(n, lbda, increment, distribution))
    return out_list

def gen_many_simulations_varyingN(lbda, increment, distribution='lognormal', num_simulations=10000):
    out_list = []
    for i in tqdm(range(num_simulations)):
...
print(run_1_simulation(3,0.1,0.1))

# out_dicts = gen_many_simulations(3,0.01,0.1, num_simulations=10000)
# out_dicts = [d for d in out_dicts if d['2']!=0.0]
# print(out_dicts[:100])
# pickle.dump(out_dicts, open('out_dicts_3_0.1_0.1.p', 'wb'))

out_dicts = gen_many_simulations_varyingN(0.01,0.1, num_simulations=10000)
out_dicts = [d for d in out_dicts if d['2']!=0.0]
print(out_dicts[:100])
pickle.dump(out_dicts, open('out_dicts_varyingN_0.01_0.1.p', 'wb'))