...
def gen_values(n, distribution='lognormal'):
    if distribution=='uniform':
        ...
        return np.random.lognormal(1,0.1,n)
    elif distribution=='beta':
        return np.random.beta(0.9,0.9,n)
    elif distribution=='beta_bimodal':
        return np.random.beta(1.1,5,n)/2 + np.random.beta(20,2,n)/2
    elif distribution=='normal':
        V1 = np.random.normal(1,4,n)
        V1 = V1[V1>0.4]
        return V1
